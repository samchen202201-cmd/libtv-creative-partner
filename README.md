...
LibTV Creative Partner 是一个面向 AI 视频创作者的编剧、导演与制作准备 Skill。它把故事意图整理成剧本、人物与场景资产、分镜、光影、运镜和可复制的生成 Prompt；剧本确定后，再交付一份从剧本到视频的完整 Word 制作指南。

版本变化见 [CHANGELOG.md](CHANGELOG.md)。

**剧本确定后，直接给你一份完整的 Word 制作指南。** 确认采用完整剧本后说“好，下一步”，即可进入整份 `.docx` 的交付，不必逐项要求分镜、资产和 Prompt。
...
确认版剧本、导演与视听设计、分镜表、全部必要角色／场景／道具／风格资产及生成 Prompt、逐镜关键帧和视频 Prompt、参考连接说明、音色与对白表演、音乐和音效安排，以及从资产准备、视频生成到剪辑混音和导出的制作说明，放进同一份 Word 文件。
...
仅选择故事方向时先写剧本；你明确只要局部内容或指定其他格式时，按你的要求交付。Word 默认指现代 `.docx` 格式。环境确实缺少文档生成或写入条件时，先保留完整正文并说明“Word 尚未导出”，不会把应急 Markdown 宣称为 Word 已完成；你明确只接受 Word 时遵循该格式要求。详见 [应急交付](references/complete-word-guide.md#document-fallback)。

默认指南采用紧凑首页、分类资产、全片时间轴和逐镜 Prompt 区块；保留确认剧本全文，不套固定镜头数或页数。包内生成器复用同一版式，让内容创作与排版分开；有你指定的模板时优先采用。普通创作不依赖 Python，使用内置 Word 生成器需要 Python 3、python-docx 和支持中文的字体，详见 [生成说明](references/word-builder.md)。
...
交付 Word 后，聊天里会附作品与计划时长、镜头／资产概览和下一步可复制指令。只显示文档中真实已有的信息，展示指令本身不自动启动生图。
...
```text
$libtv-creative-partner 就用这版完整剧本，下一步。请交付从剧本到视频制作的完整 Word 指南，所有必要资产和逐镜 Prompt 放在同一个文件里。
```
